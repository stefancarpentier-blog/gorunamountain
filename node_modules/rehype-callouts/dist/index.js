...
import { calloutRegex, splitByNewlineRegex, defaultClassNames, createIfNeeded, getConfig, expandCallouts, handleBrAfterTitle, findFirstNewline, mergeConsecutiveTextNodes, getProperties, getIndicator, getFoldIcon, } from './utils.js';
/**
...
const rehypeCallouts = (options) => {
    ...
    return (tree) => {
        visit(tree, 'element', (node) => {
            ...
            if (!isElement(node.children[0], 'p'))
                return;
            // empty paragraphs
            ...
            if (borderingIndex !== -1) {
                ...
                if (splitMatch?.groups) {
                    ...
                    if (suffix) {
                        const newParagraph = h('p', suffix, node.children[0].children.slice(borderingIndex + 1));
                        node.children = [
            ...
            // get callout type
            const revisedType = lowerType in callouts && !(lowerType in aliasMap)
                ? lowerType
                : aliasMap[lowerType];
            // get props
            const containerProperties = createIfNeeded(containerProps, node, revisedType);
            ...
            const titleIconProperties = createIfNeeded(titleIconProps, node, revisedType);
            const titleTextProperties = createIfNeeded(titleTextProps, node, revisedType);
            const foldIconProperties = createIfNeeded(foldIconProps, node, revisedType);
            // get title and collapsable
            const newFirstParagraph = node.children[0];
            if (!isElement(newFirstParagraph))
                return;
            ...
            newFirstParagraph.properties = getProperties(titleTextProperties, defaultClassNames.titleText);
            // handle container
            ...
            node.properties = getProperties(containerProperties, defaultClassNames.container);
            ...
            const fallbackTitle = theme === 'github' || theme === 'obsidian'
            ...
            // update hast
            node.children = [
                h(collapsable ? 'summary' : nonCollapsibleTitleTagName, getProperties(titleProperties, defaultClassNames.title), [
                    showIndicator
                        ? getIndicator(callouts, revisedType, titleIconTagName, titleIconProperties)
                        : null,
                    newFirstParagraph.children.length > 0
                        ? newFirstParagraph
                        : h(titleTextTagName, getProperties(titleTextProperties, defaultClassNames.titleText), customizedTitle === ''
                            ? fallbackTitle
                ...
                h(contentTagName, getProperties(contentProperties, defaultClassNames.content), node.children.slice(1)),
            ];
        });
    };
};
